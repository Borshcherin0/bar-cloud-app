...
function showEventDetail(eventId) {
    ...
    const html = `
        <div style="margin-bottom:20px;">
            ...
            ${event.location ? `<div style="font-size:0.9em;color:var(--text-secondary);margin-bottom:12px;">📍 ${esc(event.location)}</div>` : ''}
            ${event.description ? `<div style="font-size:0.9em;color:var(--text);line-height:1.5;">${esc(event.description)}</div>` : ''}
        </div>
    `;

    showGuestModal('📅 Событие', html);
}
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <button class="btn-outline" onclick="closeGuestModal()" style="margin-top:12px;">Закрыть</button>
            </div>
        `;
